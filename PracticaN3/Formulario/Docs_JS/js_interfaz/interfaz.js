...
function validarLetras(elemento) {
    ...
    if(elemento.value.length > 0){
        for (var i = 0; i < elemento.value.length; i++) {
            ...
            //Codigo Ascii de todas las letras minusculas, mayusculas y el espacio.
...
function dividirCadenas (elemento, id) {
    //obtenemos los elementos separados por espacio
    ...
    if(elemento.value.length > 10){
        ...
        if ((provincia > 0 && provincia <= numeroProviancias) && digitoTres < tercerdigito) {  
            ...
            if(digito_validador == ultimo_digito){
                ...
                elemento.value = elemento.value.substring(0, 10);
                document.getElementById('mensajeCedula').innerHTML = 'Cedula Incorrecta, vuelva a ingresarla';
                return false;
            }            
        }  else{
            elemento.value = elemento.value.substring(0, 10);
...
function numeroTelefono(elemento){
    //Aqui se validad si la cadena cumple con 10 caracteras o 7.
    if((elemento.value.length === 10) || (elemento.value.length === 7)){
        ...
        elemento.value = "";
        document.getElementById('mensajeTelefono').innerHTML = 'El telefono debe tener maximo de 10 nómeros';
        return false;
...
function validacionFechaNacimiento(elemento){
    ...
    var anio = fecha[2];  
    var dmax;
    //Comparo el tamaño  de la cadena del dia, mes y anio
    if ((dia.length == 2) && (mes.length == 2) && (anio.length == 4)) {   

        if((mes <= 12 && mes >=1) && (anio >= 1000 && anio <=3000)){
            //Convieto el mes en una variable int, y agrego un swtich para cada mes
            switch (parseInt(mes)) {        
                case 1:
                    dmax = 31;break;        
                case 2: 
            ...
            if(dia > 0 && dia < dmax ){
                ...
                document.getElementById('mensajeFecha').innerHTML = 'Fecha ingresada correcta';
                document.getElementById('mensajeFecha').style.color = '#00BB2D';
            }else{
                document.getElementById('mensajeFecha').innerHTML = 'Dia de la fecha erronea';
            }
        }else{
...
function validacionCaracteresCorreo(elemento) {
    ...
    if(elemento.value.length > 0){
        for (var i = 0; i < elemento.value.length; i++) {
            miAscii = elemento.value.charCodeAt(i);
            //Codigo ascii de numeros, arroba, letras mayuscular y minusculas, y el punto.
 ...
 /* Se debe validar que la contraseña ingresada tenga mínimo 8 caracteres, además, debe incluir al menos: una letra mayúscula, una letra minúscula
...
function validacionContrasenia(elemento) {
    ...
    if(longuitudPaswd > 8){
        ...
        //Si cada variable tiene mas de 0, eso significa que cumple con todos los requisitos para la contraseña
...
//Validar los caracteres de la contrasena
